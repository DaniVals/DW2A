...
// node_modules/@k4a_l/dirtreeist/dist/modules/parse.js
var extractText = (content3) => {
  if (!content3)
    return "";
  if (content3.type === "text")
    return content3.value;
  if ("children" in content3) {
    return content3.children.map((child) => {
...
var extractListItem = (listItem) => {
  const name = extractText(listItem.children[0]);
  const children = listItem.children.length > 1 ? extractList(listItem.children[1]) : [];
  return { name, children };
};
var extractList = (list2) => {
  if (list2.type !== "list")
    return [];
  if (!list2.children)
    return [];
  return list2.children.map((listItem) => {
...
var parse2 = (chunk) => {
  ...
  const result = lists.map((list2) => {
    try {
      return extractList(list2);
    } catch (error) {
      return [];
    }
  });
...
// node_modules/@k4a_l/dirtreeist/dist/constants/constant.js
var symbolSets = {
  normal: {
    vertical: "\u2502",
    horizontal: "\u2500",
...
var treeTypeValues = ["normal", "bold", "ascii"];

// node_modules/@k4a_l/dirtreeist/dist/modules/options.js
var optionGuard = (value, defaultValue, key) => {
  if (key === "spaceSize") {
...
var pickOption = (option, defaultOption, key) => {
  if (option === void 0)
...
var buildOption = (options, defaultOptions2) => {
  return Object.fromEntries(Object.entries(defaultOptions2).map(([key, value]) => {
    return [
      key,
      pickOption(options, defaultOptions2, key)
    ];
  }));
};

// node_modules/@k4a_l/dirtreeist/dist/modules/convert.js
var makeSymbol = (symbolSet, isLast) => {
  if (isLast)
    return symbolSet.end;
  return symbolSet.crossing;
};
var makeAsciiSpace = (spaceSize) => {
  return new Array(spaceSize).fill(" ").reduce((prev, cur) => {
    return prev + cur;
  }, "");
};
var makeSpace = (spaceSize) => {
  return new Array(Math.floor(spaceSize / 2)).fill("\u3000").reduce((prev, cur) => {
    return prev + cur;
  }, "") + (spaceSize % 2 == 1 ? " " : "");
};
var reduce = (dirTree, options, prefix, hie, isLastGroup) => {
  let result = ``;
  dirTree.some((dirNode, dirNodeIndex) => {
    ...
    result += currentLine + "\n" + childrenLines + (options.emptyBeforeUpperHierarche && isLast && !(isLastGroup || hie == 0) ? `${symbolSet.vertical}
` : "");
  });
  return result.slice(0, hie === 0 ? -1 : result.length);
};
var convert = (dirTree, options) => {
  try {
...
// node_modules/@k4a_l/dirtreeist/dist/index.js
var dirtreeist = (markdown, option) => {
  return parse2(markdown).map((dirtree) => convert(dirtree, option));
};
...
var Dirtreeist = class extends import_obsidian.Plugin {
  async onload() {
...
var DirtreeistSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Tree type").addDropdown((text3) => text3.addOptions(treeTypeOptions).setValue(this.plugin.settings.treeType).onChange(async (value) => {
      this.plugin.settings.treeType = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Insert empty line before upper hierarche").addToggle((text3) => text3.setValue(this.plugin.settings.emptyBeforeUpperHierarche).onChange(async (value) => {
      this.plugin.settings.emptyBeforeUpperHierarche = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Insert space before Name").addToggle((text3) => text3.setValue(this.plugin.settings.spaceBeforeName).onChange(async (value) => {
      this.plugin.settings.spaceBeforeName = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Space size").addDropdown((text3) => text3.addOptions({ "1": "1", "2": "2", "3": "3", "4": "4" }).setValue(String(this.plugin.settings.spaceSize)).onChange(async (value) => {
